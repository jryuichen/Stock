...
namespace PolyAndCode.UI
{
    ...
    public class VerticalRecyclingSystem : RecyclingSystem
    {
        ...
        public VerticalRecyclingSystem(RectTransform prototypeCell, RectTransform viewport, RectTransform content,
            IRecyclableScrollRectDataSource dataSource, bool isGrid, int coloumns, bool aspectRatio = true)
        {
        ...
        private void CreateCellPool()
        {
            ...
            _cellHeight = AspectRatio
                ? PrototypeCell.sizeDelta.y / PrototypeCell.sizeDelta.x * _cellWidth
            ...
            if (IsGrid) _bottomMostCellColoumn = (_bottomMostCellColoumn - 1 + _coloumns) % _coloumns;
        ...
        public override Vector2 OnValueChangedListener(Vector2 direction)
        {
            ...
            if (direction.y > 0 && _cellPool[bottomMostCellIndex] != null &&
                _cellPool[bottomMostCellIndex].MaxY() > _recyclableViewBounds.min.y)
                return RecycleTopToBottom();
            if (direction.y < 0 && _cellPool[topMostCellIndex] != null &&
        ...
        private Vector2 RecycleTopToBottom()
        {
            ...
            var n = 0;
            var posY = IsGrid ? _cellPool[bottomMostCellIndex].anchoredPosition.y : 0;
            float posX = 0;
            ...
            if (IsGrid)
            {
                ...
                if (additionalRows > 0) n -= additionalRows;
            }
        ...
        private Vector2 RecycleBottomToTop()
        {
            ...
            var n = 0;
            var posY = IsGrid ? _cellPool[topMostCellIndex].anchoredPosition.y : 0;
            float posX = 0;
        ...
        public float ScrollToTarget(int targetIndex, float viewportAlign = 0.5f, float elementAlign = 0.5f)
        {
            var cellsCount = _cellPool.Count;
            var contentHeight = cellsCount * _cellHeight;
            var viewportHeight = Viewport.rect.height; // Viewport 高度
            if (contentHeight <= viewportHeight) return 1; // Cell Pool 高度比 Viewport 高度小，不需要捲動，對齊上方就好

            var itemsCount = DataSource.GetItemCount();
            var toIndex = targetIndex;
            if (targetIndex < 0) toIndex = 0;
            if (targetIndex >= itemsCount) toIndex = itemsCount - 1;

            viewportAlign = Mathf.Clamp01(viewportAlign);
            elementAlign = Mathf.Clamp01(elementAlign);

            var topIndexOffset = toIndex; // 目標元素距離最頂元素有多少元素（不包含目標元素）
            var bottomIndexOffset = itemsCount - toIndex - 1; // 目標元素距離最底元素有多少元素（不包含目標元素）
            var cellCenterIndex = (int)((cellsCount - 0.5f) * viewportAlign); // 目標元素在 Cell Pools 的相對位置
            var topHalfCellsCount = cellsCount - cellCenterIndex - 1; // 目標元素在 Cell Pools 上半部的元素個數（不包含目標元素）
            var bottomHalfCellsCount = cellCenterIndex; // 目標元素在 Cell Pools 下半部的元素個數（不包含目標元素）

            int bottomCellsCount; // 目標元素在 Cells 中底下的元素數量（校準 Content 位置用）
            int itemIndex; // 即將要抽換 Cell 的 Index

            if (topIndexOffset < topHalfCellsCount) // 靠近整個列表頂部
            {
                itemIndex = 0;
                bottomCellsCount = cellsCount - topIndexOffset - 1;
            }
            else if (bottomIndexOffset < bottomHalfCellsCount) // 靠近整個列表底部
            {
                itemIndex = itemsCount - cellsCount;
                bottomCellsCount = bottomIndexOffset;
            }
            else // 列表中間
            {
                itemIndex = toIndex - topHalfCellsCount;
                bottomCellsCount = bottomHalfCellsCount;
            }

            // 從最上面的 Cell 開始更新
            var cellIndex = topMostCellIndex;
            for (var i = 0; i < cellsCount; i++)
            {
                DataSource.SetCell(_cachedCells[cellIndex], itemIndex);
                itemIndex++;
                cellIndex = cellIndex + 1 < cellsCount ? cellIndex + 1 : 0;
            }

            // 目前最底層 Cell 所代表的 Item Index
            currentItemCount = Math.Max(Math.Min(toIndex + bottomHalfCellsCount + 1, itemsCount), cellsCount);

            // 計算 Content 相對位置（normalized）
            var offset = contentHeight - viewportHeight;
            var bottomCellsHeight = _cellHeight * (bottomCellsCount + elementAlign);
            var viewportOffset = viewportHeight * viewportAlign;
            var moveDistance = bottomCellsHeight - viewportOffset;
            var cellPortion = Mathf.Clamp01(moveDistance / offset);
            return cellPortion;
        }

        public float GetCellViewportAlign(RectTransform cellRect, float verticalNormalizedPosition)
        {
            var cellPosition = cellRect.anchoredPosition;
            var cellsCount = _cellPool.Count;
            var contentHeight = cellsCount * _cellHeight;
            var viewportHeight = Viewport.rect.height;
            var offset = contentHeight - viewportHeight;
            var topCellsHeight = -cellPosition.y + 0.5f * _cellHeight;
            var bottomCellsHeight = contentHeight - topCellsHeight;
            var scrollOffset = offset * verticalNormalizedPosition;
            var cellViewportHeight = bottomCellsHeight - scrollOffset;
            return cellViewportHeight / viewportHeight;
        }
